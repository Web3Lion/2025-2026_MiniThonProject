 ...
 * Data Store
 *
 * For a high school project this uses localStorage + server-side JSON.
 * For production, swap these functions for database calls (Supabase, PlanetScale, etc.)
 *
 * Server-side data lives in /data/store.json (auto-created)
 * Client-side mirrors state via localStorage for offline preview
 */

import { Team, AdminState, DonationEntry, TeamMember, TierLevel } from "@/types";
...
const DEFAULT_STATE: AdminState = {
  ...
  traitPool: [],
  tokenId: null,
  collectionName: "Minthon 2025 — Cure Kids Cancer",
  totalDonations: 0,
...
export function writeStore(state: AdminState): void {
  try {
    ...
    state.totalDonations = state.teams.reduce(
      (sum, t) => sum + t.donationTotal,
      0
    );
...
export function updateMemberWallet(
  teamId: string,
  memberId: string,
  walletAddress: string
): boolean {
...
export function recordDonation(
  ...
  team.donationLog.push(entry);
  team.donationTotal += amountCents;
  team.currentTier = getTierForDonation(team.donationTotal, store.tiers)
    .level as TierLevel;
  team.updatedAt = new Date().toISOString();
  writeStore(store);
  return entry;
...
export function removeDonation(teamId: string, entryId: string): boolean {
  ...
  const team = store.teams.find((t) => t.id === teamId);
  if (!team) return false;
  const entry = team.donationLog.find((e) => e.id === entryId);
...
export function updateCollectionName(name: string): void {
  const store = readStore();
...
export function getTeamByMemberWallet(walletAddress: string): Team | null {
  const store = readStore();
  return (
    store.teams.find((t) =>
      t.members.some((m) => m.walletAddress === walletAddress)
    ) ?? null
  );
}
...
export function getLeaderboard(): Team[] {
  return [...readStore().teams].sort(
    (a, b) => b.donationTotal - a.donationTotal
  );
}
